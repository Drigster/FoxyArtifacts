    ...
    public void onEntityDeath(EntityDeathEvent e) {
        ...
        if (dead.getType().equals(EntityType.FOX)) {
            ...
            if (killer instanceof Player) {
                if (((Player) killer).getGameMode().equals(GameMode.CREATIVE)) {
                    return;
                }
                Bukkit.broadcastMessage(killer.getName() + " вызвал небесную кару!");
                int r = 4;
                Location l = location.subtract(r, 0, r / 2);
                for (double i = 0; i < 2 * Math.PI; i = i + (Math.PI / 3)) {
                    ...
                    fox.setGoalTarget((EntityLiving) ((CraftPlayer) killer).getHandle());
                    world.addEntity(fox);
                    foxes.add(fox);
                }
                ...
                for (foxGod fox : foxes) {
                    scheduler.runTaskLater(plugin, new Runnable() {
                        @Override
                        public void run() {
                            ((CraftPlayer) killer).playSound(fox.getBukkitEntity().getLocation(), Sound.ENTITY_ILLUSIONER_CAST_SPELL, 2, 1);
                            ((CraftPlayer) killer).spawnParticle(Particle.EXPLOSION_NORMAL, fox.locX(), fox.locY(), fox.locZ(), 60);
